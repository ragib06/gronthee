...
- **Already-registered email on sign-up**: Supabase deliberately obfuscates this case to prevent email enumeration: `signUp` returns a "fake" user with `session: null`, no email is sent, but `data.user.identities` is an empty array. The Create-account flow checks `data.user?.identities?.length === 0` and surfaces a clear message ("This email is already registered. Sign in, or use 'Forgot password?' to set a new password.") instead of the misleading "confirmation link sent". Existing magic-link-only users add a password via the Forgot password flow, which authenticates them through the recovery link and lets them set one.
