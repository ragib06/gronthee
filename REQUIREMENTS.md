...
- **AI Analysis**: Uses third party AI models to analyze one or multiple images of a book. It should support multiple providers (i.e Claude, GPT, Gemini, OpenRouter). The API keys are stored as Vercel environment variables (`MONDOL_VITE_ANTHROPIC_API_KEY`, `VITE_OPENAI_API_KEY`, `MONDOL_VITE_GEMINI_API_KEY`, `MONDOL_VITE_OPENROUTER_API_KEY`) and read at runtime via `import.meta.env`. Users get to choose a model from a dropdown list before scanning. OpenRouter models (e.g. Qwen 3.6 Plus, model ID `qwen/qwen3.6-plus:free`) are accessed via the OpenAI-compatible API at `https://openrouter.ai/api/v1`. Gemma 4 (model ID `gemma-4-31b-it`) is available under the Google Gemini provider and is accessed via the `@google/generative-ai` SDK.
