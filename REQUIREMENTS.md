...
- **CSV Export**: Allows users to export their entire scanning history into a CSV file with all metadata fields. Clicking "Export CSV" opens a modal dialog with an editable filename field pre-filled with `gronthee-<username>-<datetime>.csv` (using the stored username and the current date/time formatted as `yyyy-mm-dd-hh-mm-ss`). The user can edit the name and click "Save" to trigger the download, or cancel. If the filename lacks a `.csv` extension it is appended automatically. The `Collection` and `Item Type` columns use abbreviated codes (e.g. `FIC`, `BK`) not human-readable labels.
  - **R&D columns (optional)**: The Export CSV dialog includes an "Include R&D columns" checkbox (unchecked by default). When checked, three additional columns are appended to the end of each exported CSV: `Book ID` (the internal book UUID), `Images` (JSON-encoded array of public R2 URLs, e.g. `["…/1.jpg","…/2.jpg"]`, empty string for books with no uploaded images), and `Prompt Output` (the JSON body returned by the AI for that scan, with any surrounding markdown fences or prose stripped — stored as a string starting with `{` and ending with `}`; empty for pre-feature books or books saved without a scan). The downloaded filename is prefixed with `rnd-` in this mode: `gronthee-rnd-<username>-<sessionSlug>-<datetime>.csv`. These columns are intended for research and data-quality analysis and are omitted from the default export so the main CSV continues to match the external library system's schema. The raw AI output is captured at scan time and persisted on the `BookMetadata` record under `rawAIOutput`.
